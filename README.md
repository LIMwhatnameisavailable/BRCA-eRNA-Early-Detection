...
| :--- | :--- | :--- |
| `Annotation_eRNA_in_Super_enhancer_all.bed` | ~300k eRNA loci within ENCODE super-enhancer regions; used to generate the SAF quantification file | **TCeA** |
| `eRNA_standard_500bp.saf` | Custom SAF file with each eRNA locus symmetrically expanded ±500 bp; directly used by `featureCounts` in Script 0 | Generated from the `.bed` file above |
| `SraRunTable_GSE225846.csv` | SRA run metadata for external cohort 1; used for sample annotation and stage filtering | **NCBI SRA** (GEO: GSE225846) |
| `SraRunTable_GSE229571.csv` | SRA run metadata for external cohort 2; used for sample annotation and stage filtering | **NCBI SRA** (GEO: GSE229571) |
| `Normal_like_samples.csv` | Purified normal reference sample list after GTEx-based field cancerization filtering | Generated by `Prognosis_R/R_Scripts/8_Quality_Control_for_Normal_Like_References.R` |
| `TCGA-BRCA.clinical.tsv` | Phenotype data used for Stage I/II filtering | **UCSC Xena** · GDC TCGA Breast Cancer (BRCA) |
...
The expression matrices for GSE225846 and GSE229571 are **not pre-computed**.
