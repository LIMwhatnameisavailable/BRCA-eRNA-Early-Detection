...
def preprocess_split(df_train, df_test_list, method='robust_log1p'):
    ...
    if method == 'robust_log1p':
        # Log1p 变换后接 RobustScaler + Normalizer
...
if HAS_TORCH:
    class SEBlock(nn.Module):
        """通道注意力模块（Squeeze-and-Excitation），对重要特征通道动态加权"""
    ...
    class Attention1DCNN(nn.Module):
        """1D-CNN 分类网络，使用 GroupNorm + GELU + SE 注意力模块"""
        def __init__(self, input_dim):
            super(Attention1DCNN, self).__init__()
            # GroupNorm(1, C) 等效于 LayerNorm，不受 batch size 影响，适合小样本临床场景
        ...
        def predict_proba(self, X):
            ...
            with torch.no_grad():
                return torch.softmax(
                    self.model(torch.FloatTensor(X).to(self.device)), dim=1
                ).cpu().numpy()
...
def plot_roc_bootstrap_R(dataset_name, data_list, filename, n_bootstraps=N_BOOTSTRAPS):
    ...
    plt.legend(loc='lower right', bbox_to_anchor=(0.95, 0.05), frameon=False,
...
if __name__ == "__main__":
    ...
    print("\n>>> 2. 预处理...")
    # ML 使用 robust，DL 使用 robust_log1p
    ...
    try:
        # 重建 DL scaler 并在训练集上 fit，用于网页端推理时的一致性预处理
    ...
    print(f"\n全部完成！图片、指标表格及网页所需 .pkl 文件均已保存至 {OUTPUT_DIR}")
