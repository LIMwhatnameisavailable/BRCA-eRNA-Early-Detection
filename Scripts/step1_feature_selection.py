...
def get_sample_class(row, type_col, stage_col):
    ...
    if 'normal' in tissue_val or 'healthy' in tissue_val: return "Normal"
    
    if stage_col in row and pd.notna(row[stage_col]):
        stage_val = str(row[stage_col]).upper().strip().replace("STAGE", "").strip()
        if any(x in stage_val for x in ['III', 'IV', '3', '4']): return "Late"
        if any(x in stage_val for x in ['I', '1', '2']): return "Tumor"
...
def load_external_data(config):
    ...
    try:
        ...
        # 加入RPKM转换(GSE数据窗口长度500bp)
        df_counts = df[common_ids]
        # 按列(样本)求和，然后每个 count 除以总和再乘 10^6
